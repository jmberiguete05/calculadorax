...
function calcularArea() {
    ...
    var widthFraccion = document.getElementById('widthFraccion').value.trim();
    var heightEntero = parseFloat(document.getElementById('heightEntero').value);
    var heightFraccion = document.getElementById('heightFraccion').value.trim();
 ...
 // Validar y convertir la entrada en fracción decimal a número
    ...
    if (isNaN(width) || isNaN(height)) {
        document.getElementById('resultado').innerText = 'Por favor, ingrese valores válidos para el ancho y la altura.';
        return;
    }

    var area = (width * height) / 144; // Convertir pulgadas cuadradas a pies cuadrados
    document.getElementById('resultado').innerText = 'El área de la ventana es: ' + area.toFixed(2) + ' pies cuadrados.';
...
function convertirFraccionDecimalANumero(entero, fraccion) {
    // Si la fracción está vacía, retornar solo el número entero
    if (fraccion === "") {
        return entero;
    }
